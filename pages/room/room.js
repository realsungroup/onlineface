...
Page({
  ...
  enterRoom: function(params) {
    ...
    this.setData(
      ...
      () => {
        ...
        this.trtcComponent
          .enterRoom({ roomID: params.roomID })
          .then(() => {
  ...
  bindTRTCRoomEvent: function() {
    ...
    this.trtcComponent.on(TRTC_EVENT.REMOTE_USER_JOIN, event => {
      console.log(
        "* room REMOTE_USER_JOIN",
        event,
        this.trtcComponent.getRemoteUserList()
      );
    ...
    this.trtcComponent.on(TRTC_EVENT.REMOTE_VIDEO_ADD, event => {
      ...
      if (
        this.template === "1v1" &&
        (!this.remoteUser || this.remoteUser === data.userID)
      ) {
        // 1v1 只订阅第一个远端流
